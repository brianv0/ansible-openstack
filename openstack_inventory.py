...
def getFloatingIpFromServerForNetwork(server, network):
	addrs = server.addresses.get(network)
	types = {i.get('OS-EXT-IPS:type'):i for i in addrs}
        # Prefer floating IP over fixed
	if 'floating' in types:
		return types['floating']['addr']
	if 'fixed' in types:
		return types['fixed']['addr']
	return None
